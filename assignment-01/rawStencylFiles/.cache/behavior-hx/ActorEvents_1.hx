...
class ActorEvents_1 extends ActorScript
{
	...
	override public function init()
	{
		...
		addCollisionListener(actor, function(event:Collision, list:Array<Dynamic>):Void
		{
			...
			{
				if((event.otherActor.getType() == getActorType(4)))
				{
					Engine.engine.setGameAttribute("score", ((Engine.engine.getGameAttribute("score") : Float) + 1));
					recycleActor(event.otherActor);
					Engine.engine.setGameAttribute("numberOfPointPellets", ((Engine.engine.getGameAttribute("numberOfPointPellets") : Float) - 1));
					if(((Engine.engine.getGameAttribute("numberOfPointPellets") : Float) <= 0))
					{
						Engine.engine.setGameAttribute("winState", "win");
					}
				}
			}
		});
		...
		addCollisionListener(actor, function(event:Collision, list:Array<Dynamic>):Void
		{
			...
			{
				Engine.engine.setGameAttribute("winState", "lose");
			}
		});
